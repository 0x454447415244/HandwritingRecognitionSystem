...
from util import LoadModel, ReadData, SaveModel, target_list_to_sparse_tensor
from rnn import RNN
from config import cfg
from cnn import CNN, WND_HEIGHT, WND_SHIFT, WND_WIDTH, CNNLight, MPoolLayers_H
import numpy as np
import cv2
import sys
import math
...
error_rate = tf.reduce_sum(tf.edit_distance(
    predicted, targets, normalize=False)) / tf.to_float(tf.size(targets.values))
...
grad, _ = tf.clip_by_global_norm(
    tf.gradients(loss, tvars), cfg.MaxGradientNorm)
...
OverallTrainingLoss = tf.Variable(
    0, name='OverallTrainingLoss', dtype=tf.float32)
OverallTrainingError = tf.Variable(
    0, name='OverallTrainingError', dtype=tf.float32)
OverallValidationLoss = tf.Variable(
    0, name='OverallValidationLoss', dtype=tf.float32)
OverallValidationError = tf.Variable(
    0, name='OverallValidationError', dtype=tf.float32)
OverallTrainingLoss_s = tf.summary.scalar(
    'OverallTrainingLoss', OverallTrainingLoss)
OverallTrainingError_s = tf.summary.scalar(
    'OverallTrainingError', OverallTrainingError)
OverallValidationLoss_s = tf.summary.scalar(
    'OverallValidationLoss', OverallValidationLoss)
OverallValidationError_s = tf.summary.scalar(
    'OverallValidationError', OverallValidationError)

# Reading training data...
inputList, seqLens, targetList = ReadData(
    cfg.TRAIN_LOCATION, cfg.TRAIN_LIST, cfg.TRAIN_NB, WND_HEIGHT, WND_WIDTH, WND_SHIFT, VEC_PER_WND, cfg.TRAIN_TRANS)

# Reading validation data...
if (cfg.VAL_NB > 0):
    inputListVal, seqLensVal, targetListVal = ReadData(
        cfg.VAL_LOCATION, cfg.VAL_LIST, cfg.VAL_NB, WND_HEIGHT, WND_WIDTH, WND_SHIFT, VEC_PER_WND, cfg.VAL_TRANS)
...
OverallSummary = tf.summary.merge(
    [OverallTrainingLoss_s, OverallTrainingError_s, OverallValidationLoss_s, OverallValidationError_s])

SummaryWriter = tf.summary.FileWriter(cfg.LogDir, session.graph)

if cfg.StartingEpoch != 0:
...
try:
    for epoch in range(cfg.StartingEpoch, cfg.NEpochs):
        ...
        while end <= len(inputList):
            ...
            feed = {x: batchInputs, SeqLens: batchSeqLengths, indices: batchTargetIxs,
                    values: batchTargetVals, shape: batchTargetShape}
            del batchInputs, batchTargetIxs, batchTargetVals, batchTargetShape, batchSeqLengths

            _, summary, Losses, Loss, Error = session.run(
                [train_step, LocalTrainSummary, losses, loss, error_rate], feed_dict=feed)
            del feed
            ...
            if numberOfInfElements > 0:
                ...
                LogFile.write(
                    f"{batchTargetList[np.where(np.isinf(Losses) == True)[0][0]]}\n")
        ...
        LogFile.write(
            f"Training loss: {TrainingLoss:.6f}, Training error: {TrainingError:.6f}\n")
        ...
        if (cfg.VAL_NB > 0):
            ...
            while end <= len(inputListVal):
                ...
                batchTargetSparse = target_list_to_sparse_tensor(
                    batchTargetList)
                batchTargetIxs, batchTargetVals, batchTargetShape = batchTargetSparse
            ...
            LogFile.write(
                f"Validation loss: {ValidationLoss:.6f}, Validation error: {ValidationError:.6f}\n")
            LogFile.flush()

            feed = {OverallTrainingLoss: TrainingLoss, OverallTrainingError: TrainingError,
                    OverallValidationLoss: ValidationLoss, OverallValidationError: ValidationError}

            SummaryWriter.add_summary(session.run(
                [OverallSummary], feed_dict=feed)[0], epoch)
            SummaryWriter.flush()
            del feed
            ...
            if ValidationLoss < currValLoss:
                ...
                if nTimesNoProgress == cfg.TrainThreshold:
                    session.close()
                    LogFile.write(
                        "No progress on validation. Terminating program.\n")
                    sys.exit(0)
