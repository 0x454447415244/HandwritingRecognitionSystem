...
def ReadData(filesLocation, filesList, numberOfFiles, WND_HEIGHT, WND_WIDTH, WND_SHIFT, VEC_PER_WND, transDir=''):
    ...
    with open(filesList) as listHandler:
        ...
        for imageFile in imageFiles:
            ...
            if filesLocation != '':
                tfile = imageFile.strip('\n')
            else:
                tfile = os.path.basename(imageFile.strip('\n'))
                ...
                with open(targetFile) as f:
                    ...
                    if len(data) == 0:
                        ...
                        for i in range(len(data)):
                            targetData = np.fromstring(
                                data[i], dtype=np.uint16, sep=' ')
            ...
            if filesLocation != '':
                imageFilePath = filesLocation + "/" + tfile + cfg.ImageFileType
            else:
                imageFilePath = imageFile.strip('\n') + cfg.ImageFileType
            ...
            if (h > WND_HEIGHT):
                factor = WND_HEIGHT/float(h)
            else:
                factor = 1.0

            image = cv2.resize(image, None, fx=factor,
            ...
            while True:
                ...
                if e > w:
                    ...
                    if h < WND_HEIGHT:
                        pad = np.ones(
                            ((WND_HEIGHT - h), WND_WIDTH), np.uint8)*255
