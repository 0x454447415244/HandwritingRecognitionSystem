...
flags.DEFINE_string('LabelFileType', '.tru',
...
flags.DEFINE_string('Probs', './Probs',
...
flags.DEFINE_integer('BatchSize', 10, 'Batch size')
flags.DEFINE_boolean('RandomBatches', True,
...
flags.DEFINE_integer('TrainThreshold', 20,
                     'Stop the training after n epochs with no improvement on validation')

cfg = flags.FLAGS

if (os.path.exists(cfg.SaveDir) == False):
    os.makedirs(cfg.SaveDir)
if (os.path.exists(cfg.LogDir) == False):
